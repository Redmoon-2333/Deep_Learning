...
计算偏导数：
```
∂L/∂x = (∂L/∂z) × (∂z/∂x) = 2z × 1 = 2(x + y)
∂L/∂y = (∂L/∂z) × (∂z/∂y) = 2z × 1 = 2(x + y)
```
...
则：
```
∂L/∂X = ∂L/∂Y @ Wᵀ = (B, m) @ (m, n) = (B, n) ✓
∂L/∂W = Xᵀ @ ∂L/∂Y = (n, B) @ (B, m) = (n, m) ✓
∂L/∂b = sum(∂L/∂Y, axis=0) = (m,) ✓
```
...
计算:
Y = X @ W + b
  = [[1×0.5+2×0.2, 1×0.8+2×0.3],
     [3×0.5+4×0.2, 3×0.8+4×0.3]] + [[0.1, 0.1],
                                      [0.1, 0.1]]
  = [[0.9, 1.5],
     [2.3, 3.5]]
```
...
∂L/∂aₖ = ∂L/∂yₖ × ∂yₖ/∂aₖ + Σᵢ≠ₖ ∂L/∂yᵢ × ∂yᵢ/∂aₖ
       = yₖ - tₖ
```
